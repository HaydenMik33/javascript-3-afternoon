...
//Code Here
let postTaxPrices = prices.map(a => a*1.07);// = prices.map(/* Provide Your Callback Here );
...
//Code Here
let totalPopulation  = populations.reduce((total,cur)=>total+cur);//  = populations.reduce(/* Provide Your Callback Here */)
...
//Code Here
let myStrongest= monstersInYourPocket.filter(a => a.CP>200); // = monstersInYourPocket.filter(/* Provide Your Callback Here */)
...
let ordersTotal= orders.reduce((total, curElement )=>{total["price"]*(1+total["tax"])+curElement["price"]*(1+curElement["tax"])},0);//Code Here
